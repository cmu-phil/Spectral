...
(f_2 \smsh g_2)\o (0 \smsh g_1)\arrow[d,equals,"(f_2\smsh g_2)\o q"] \\
0\smsh (g_2 \o g_1) \arrow[r,equals,"q"] &
0
\end{tikzcd}
\end{center}
...
\begin{proof}
	...
	\end{align*}		
	where the last homotopy is given by $(A \to c) \o t \sim c : (\bool \to A \to X) \to (A \to X)$.
\end{proof}
...
%The order of arguments are different in the formalization here and there. Also, some smashes are commuted. This is because some unfortunate choices have been made in the formalization. Reversing these choices is possible, but probably more work than it's worth (the final result is exactly the same).
